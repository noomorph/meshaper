        ...
        return (
            <div className={classnames(css.app, this.props.className)}>
                <div key="navigationBar" className={classnames(css['bottom-bar'], css['bottom-bar-three'])}>
                    <Link to="/" activeClassName={css.active} className={classnames(css['bottom-bar-link'], css['icon-home'])}>{i18n('NAV_TODAY')}</Link>
                    <Link to="/training" activeClassName={css.active} className={classnames(css['bottom-bar-link'], css['icon-list'])}>{i18n('NAV_CALENDAR')}</Link>
                    <Link to="/training" activeClassName={css.active} className={classnames(css['bottom-bar-link'], css['icon-menu'])}>{i18n('NAV_ANALYSIS')}</Link>
                </div>
                {this.props.children}
            </div>
        );
